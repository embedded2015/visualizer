...
#define traceTASK_CREATE( pxNewTCB ) trace_task_create((pxNewTCB), (pxNewTCB)->pcTaskName, (pxNewTCB)->uxPriority);
...
#define traceTASK_SWITCHED_IN() \
	if (pxPreviousTCB != pxCurrentTCB) { \
		trace_task_switch(pxPreviousTCB, previous_systick_current, pxCurrentTCB); \
	}

#define traceCREATE_MUTEX( pxNewQueue ) trace_create_mutex(pxNewQueue);

#define traceQUEUE_CREATE( pxNewQueue ) trace_queue_create(pxNewQueue, ucQueueType, uxQueueLength);

#define traceQUEUE_SEND( pxQueue ) trace_queue_send(xTaskGetCurrentTaskHandle(), pxQueue);

#define traceQUEUE_SEND_FROM_ISR( pxQueue ) trace_queue_send((void*)get_current_interrupt_number(), pxQueue);

#define traceQUEUE_RECEIVE( pxQueue ) trace_queue_recv(xTaskGetCurrentTaskHandle(), pxQueue);

#define traceQUEUE_RECEIVE_FROM_ISR( pxQueue ) trace_queue_recv((void*)get_current_interrupt_number(), pxQueue);
